...
<article class="web-product-card">
    <a class="web-product-card-link" href="{{ route('site.products.show', $product) }}" aria-label="{{ $translation?->name ?? $product->sku }}">
        <div class="web-product-media">@if($image)<img src="{{ Storage::disk('public')->url($image) }}" alt="{{ $translation?->name ?? $product->sku }}">@else<span>{{ $account?->name ?? 'Landivo' }}</span>@endif @if($discount)<b>-{{ $discount }}%</b>@endif</div>
